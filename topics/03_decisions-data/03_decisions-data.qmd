...
::: {.fragment}
We can organize this information into a decision matrix (i.e., a **loss** or **utility** matrix), assuming we know the outcomes
:::
...
|-----------------|------------|------------|------------|------------|
| **Actions**     | Low        | Medium     | High       |            |
| Price at \$4.00 | -\$20,000  | \$30,000   | \$70,000   | *\$37,000* |
| Price at \$3.50 | -\$15,000  | \$25,000   | \$75,000   | *\$36,500* |
| Price at \$3.00 | -\$10,000  | \$20,000   | \$80,000   | *\$35,000* |
...
**Decision theory** (a.k.a., **decision science**) provides a principled, unifying framework for informing decision-making under uncertainty
...
::: {.incremental}
- Our **profit function** $P(p, D) = D(p) \times (p - c)$ is a linear utility function
- Linear utility is also **symmetric** and assumes the decision-maker is **risk neutral**
:::

# How does misspecifying the objective function relate to the "alignment" problem? {background-color="#006242"}
...
[Simulating data is ***forward* generative modeling** as captured by $f(X, Y | \beta)$]{.fragment}

# Copy the code to simulate data and try different parameter values. How does the simulated data change? {background-color="#006242"}
...
::: {.fragment}
For interpretable models, we **assume a mapping function exists** and try to carefully build one so we can focus on quantifying uncertainty by estimating parameters
:::
...
7. Submit both your Quarto document and your code and printed output as a PDF on Canvas
