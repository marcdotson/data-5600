...
1.  Harmon’s has made a portion of their loyalty CRM data available:
    Consider what should be included in the model
2.  Iterate on the model using these new predictors, including EDA,
    checking model assumptions, and feature engineering
3.  Fit the model using the additional data using OLS and, if you can, a
    Bayesian model with uniform priors
4.  Interpret the interval estimates, being careful to track transformed
    scales, reference levels, the presence of multiple predictors in the
    model, and the differences between credible and confidence intervals
5.  Submit your code, output, and interpretations as a single PDF on
    Canvas
