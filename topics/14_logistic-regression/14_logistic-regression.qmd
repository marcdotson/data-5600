...
## Using OLS diagnostics

We can still use OLS diagnostics for logistic regression **when we don't have a logistic regression-specific diagnostic**
...
- However, we can still run an OLS model to compute VIF for a model with a binary outcome since our its based on the relationship among predictors and the way the model is estimated isn't relevate to estimating collinearity
:::
