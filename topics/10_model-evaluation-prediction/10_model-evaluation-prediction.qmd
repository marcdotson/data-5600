...
::: {.fragment}
There is no guarantee that statistical or predictive criteria are consistent with what is optimal for informing our decision; at best, they are **proxies for decision-theoretic fit**
:::
...
![](../../figures/lm_in-sample-fit.png){fig-align="center"}

## 

![](../../figures/meme_r-squared.png){fig-align="center"}
...
- Still not great, but there is a predictive fit version
:::
...
![](../../figures/meme_adjusted-r-squared.png){fig-align="center"}

## In-sample fit: F-statistic

The F-statistic is the test statistic of the null hypothesis that **all regression coefficients are equal to zero**, so what does a small $p$-value or Prob(F-statistic) mean?

::: {.incremental}
- The smaller the $p$-value, the stronger the evidence that at least one predictor has a non-zero coefficient (i.e., is useful for explaining $y$)
...
Following our original model (i.e., without loyalty data)
...
- RMSE can be interpreted with the same units of the outcome variable
...
X_new = (pl.DataFrame({
    ...
    'size_12': [1, 1, 1, 0, 0, 0],
    'price': [3.00, 3.50, 4.00, 3.00, 3.50, 4.00]
  }).with_columns(
      pl.col('price').log().alias('log_price')
  ).select(pl.exclude('price'))
).to_pandas()
...
- When we make predictions, be careful with **extrapolation** or making predictions outside the range of the data used to fit the model
:::
...
1. Compare the models from the previous two exercises: the model that didn't include the loyalty data and the model that did
2. Choose the best-fitting model and re-run it using the full dataset
3. Update your interpretation of the interval estimates and produce predictions relevant to the business objective using this final, best-fitting model
4. Submit your code, output, and commentary as a single PDF on Canvas
