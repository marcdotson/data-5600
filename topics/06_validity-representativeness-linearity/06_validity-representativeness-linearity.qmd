---
title: "Validity, Representativeness, <br>and Linearity"
...
## Scatterplot of outcome and predictor(s) {visibility="uncounted"}
...
(so.Plot(pb_data, x = 'log_price', y = 'units')
    .add(so.Dot(alpha = 0.25))
    .add(so.Line(), so.PolyFit())
)
```
...
(so.Plot(pb_data, x = 'price', y = 'log_units')
    .add(so.Dot(alpha = 0.25))
    .add(so.Line(), so.PolyFit())
)
```
...
::: {.incremental}
- Partial regression plots attempt to show the effect of adding each new predictor to the model (given that one or more predictors are already in the model)
- For each new predictor, we take whatever is leftover after the fit of the other predictors and look to see if adding the new predictor matters
:::

::: {.fragment}
If the linearity assumption is met, each of the partial regression plots should be roughly linear
:::
...
- Quickly reviewed exploratory data analysis
