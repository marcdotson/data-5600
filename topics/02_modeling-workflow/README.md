...
# Import data
sl_data = pl.read_parquet(here('data', 'original_df.parquet'))

# Peanut butter purchases
pb_data = sl_data.filter(pl.col('units') > 0)

# Visualize price and units
(ggplot(pb_data, aes(x = 'price', y = 'units'))
...
Start working through the Plan and Build steps of the modeling workflow.
What is the business objective? What is the data story? What sort of
loss function captures the business objective? What would be the ideal
data and how might you include it in a likelihood function?
