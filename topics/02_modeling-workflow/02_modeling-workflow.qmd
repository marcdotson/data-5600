...
::: {.column .gray-box width="92%"}
If we assume $\epsilon$ follows a probability distribution, we can use our model (i.e., likelihood function) to **simulate data**
...
def sim_data(n, beta_0, beta_price):
    ...
    y = (beta_0 + beta_price * price + error).round(0)
    return price, y
...
# What ideas does your group have for your first project? What is the business objective and the data story? {background-color="#006242"}
...
::: {.fragment}
It is unlikely that the data is **ideal**, however it is **your job** to understand the data so you can communicate its strengths and limitations
:::
:::
...
In the case, the transaction data contains the critical variables, units sold and price, but it doesn't contain other variables we might want, like
...
- We built a probabilistic model, which includes unknown parameters that represent the information we are uncertain about
:::
...
## Test {auto-animate=true visibility="uncounted"}
...
# What's your preferred task when working with data? Is it data wrangling, visualizing data, or implementing a specific method? {background-color="#006242"}
...
5. Finish setting up the data stack
