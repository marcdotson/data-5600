...
::: {.fragment}
For different prices $p$ (actions), we can compute profit $P$ given demand $D$ (states of the world)
:::
:::
...
- Demand $D$ is unknown or known with uncertainty (state of the world)
:::
...
::: {.fragment}
$\theta$ is the **weights** or **parameters** of the mapping function from $X$ to $Y$ and the information we want to extract from the data
:::

::: {.fragment}
We will see that this same model is used to determine *likely* values of $\theta$ given $X$ and $Y$ and is also called a **likelihood function**
:::
:::
...
$$
y = \beta_0 + \beta_{price} X_{price} + \epsilon
$$
...
# Import data
sl_data = pl.read_parquet(here('data/soft_launch.parquet'))
...
- It is often beneficial to present predictions for different scenarios
:::
:::
...
::: {.column .gray-box width="42%"}
In the case, we discover that accounting for uncertainty in the price elasticity estimate (i.e., the parameter measuring the relationship between `price` and `units`) leads to predicted monthly profits of $146,000 to $377,300

All of the predictions are positive, which supports the decision to launch the new private label
:::
...
2. Finish setting up your [data stack](https://github.com/marcdotson/data-stack) 
