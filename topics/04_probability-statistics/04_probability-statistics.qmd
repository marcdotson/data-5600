...
If our mapping function uses probability distributions to model the data generating process or population, its called a **parametric model**
...
Our parametric model of the data $f(X | \theta)$ is used to determine *likely* values of $\theta$ and is also called a **likelihood function** $\mathcal{L}(\theta | X)$
...
::: {.incremental}
- If you give a likelihood function parameters $\theta$, it can produce plausible observations $X$
- If you give a likelihood function observations $X$, it can determine plausible parameters $\theta$
:::
