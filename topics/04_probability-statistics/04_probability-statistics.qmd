...
Our parametric model of the data $f(X, Y | \theta)$ is used to determine *likely* values of $\theta$ and is also called a **likelihood function** $L(\theta | X, Y)$ or **data distribution**

$$
\LARGE{f(X, Y | \theta) = L(\theta | X, Y)}
$$

::: {.incremental}
- $f(X, Y | \theta)$: If you give a likelihood function parameters $\theta$, it can simulate plausible data $X$ and $Y$ (i.e., **forward** generative modeling)
- $L(\theta | X, Y)$: If you give a likelihood function data $X$ and $Y$, it can determine plausible parameters $\theta$ (i.e., **backward** generative modeling)
:::

## Simulate data $f(X, Y | \theta)$

```{python}
#| code-line-numbers: "|1|3-4|6-12|14-15|17"
#| eval: true
...
data = sim_data(n = 5, beta_0 = 2, beta_x1 = -0.3, beta_x2 = 4, sigma = 5)

data
```

## Estimate parameters $L(\theta | X, Y)$

```{python}
#| code-line-numbers: "|1|3-5|7-8|10-11|13-14|16"
#| eval: true
...
::: {.incremental}
- Use Bayes' theorem to combine the likelihood function (i.e., **data distribution**) with a **prior distribution** $p(\theta)$
- This produces a **posterior distribution** $p(\theta | X)$ of the random variable $\theta$
- Draw samples from the posterior distribution using **Markov chain Monte Carlo** (MCMC)
:::
:::
...
################################################################
# Temporary fix for a mismatch between macOS xcode and pytensor.
################################################################
