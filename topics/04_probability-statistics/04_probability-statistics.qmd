...
::: {.incremental}
- $f(X, Y | \theta)$: If you give a likelihood function parameters $\theta$, it can simulate plausible data $X$ and $Y$
- $\mathcal{L}(\theta | X, Y)$: If you give a likelihood function data $X$ and $Y$, it can determine plausible parameters $\theta$
:::

## Simulate data $f(X, Y | \theta)$

```{python}
#| code-line-numbers: "|1|3-4|6-12|14-15|16"
#| eval: true
...
## Estimate parameters $\mathcal{L}(\theta | X, Y)$
