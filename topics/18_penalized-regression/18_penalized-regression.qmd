...
Point estimates are chosen by **minimizing the loss** on the training data, but that's not guarantee it will have low loss on future data (i.e., **generalize**) thus overfitting remains a problem
...
- The complexity penalty **shrinks** parameter estimes toward zero
...
Penalized regression helps us navigate the bias-variance tradeoff using shrinkage so that we can deal with overfitting, improve generalization, and learn regular features, but complexity comes with a cost
...
4. Combine correlated variables via dimension reduction
:::
...
- Computationally expensive when $p$ is large, so only possible if $p$ is relatively small
:::
...
## Stepwise methods: Stepwise/sequential replacement
