...
::: {.fragment}
This is the most common GLM known as **Binomial regression**, the **logit model**, or **logistic regression**
:::
...
Remember, we can pretend our model *is* the data generation process and **simulate data** (i.e., generate data) to prepare for our analysis
...
def sim_data(n, beta_0, beta_x1, beta_x2):
    ...
    prob_y = np.exp(beta_0 + beta_x1 * x1 + beta_x2 * x2) / (1 + np.exp(beta_0 + beta_x1 * x1 + beta_x2 * x2))
    y = rng.binomial(1, prob_y, size=n)
    return y, x1, x2

sim_data(n = 5, beta_0 = 2, beta_x1 = -0.3, beta_x2 = 4)
```
