...
::: {.incremental}
- There are $p$ predictors including the intercept so we really have $p−1$ predictors
...
- The predictors all need to be **standardized before preforming PCA**
...
$$
\begin{align}
PC_{1} &= 0.3 Q_{1} - 0.1 Q_{2} - 0.3 Q_{3} + \cdots + 0.5 Q_{8} + 0.7 Q_{9} + 0.5 Q_{10}\\
PC_{2} &= 0.7 Q_{1} + 0.6 Q_{2} - 0.2 Q_{3} + \cdots + 0.8 Q_{8} + 0.5 Q_{9} - 0.2 Q_{10}
\end{align}
$$
...
It's easiest to visualize at what point adding additional princiapl components does not add much additional information using a **scree plot** or **elbow plot**
