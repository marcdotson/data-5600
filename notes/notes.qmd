...
SLOW DOWN, VISUALIZE/ILLUSTRATE, QUESTIONS, ACTIVITIES

### Decision-Making in the Presence of Uncertainty

#### Introduce decision theory/science

Decision theory is a principled, unifying framework for informing decision making in the presence of uncertainty

- Not everyone does use decision theory, but you can -- let's improve practice
...
#### Illustrate first with decision-making under *certainty* with a payoff/loss matrix
...
#### Move to decision-making in the presence of *uncertainty*

- Example loss functions -- compare symmetric/linear and non-linear loss
...
- Equivalence of utility and loss functions: $l(a, s) = - u(a, s)$
- Build up to decision-making under uncertainty using the case
- Uncertainty abou the state of the world is what motivates probability and statistics
- Activity: What are the actions? What is the state of the world?
...
- Mapping as real world maps -- story --> functional form
- Assume a functional mapping exists. Predictive. Interpretable. Causal.
- Clarity about interpretable models vs. black box models

- Make a simple story simple but no simpler
- Any model is a simplification of reality (small world vs. larger world, model rocket vs. actual), but the goal is to capture the essence of the data generation process
- Models are simplifications of reality, but the goal is to capture the essence of the data generation process
- Models are abstractions of reality -- so how do you know if a model is a good model?
...
- Intercept-only model -- no story
- A statistical model is descriptive, about assocations, not (necessarily) causation
...
- Activity: White board a simple data generating process for your project

### Decision-Maing in the Presence of Uncertainty as a Motivation for Probability

- Complemented by a probabilistic approach to machine learning (expected loss)
- Probability as the unifying mathematical language of uncertainty
- Probability as frequency assumes symmetry and repeatability
- Frequentist (objective) vs. Bayesian (subjective) interpretations of probability
- Interpretable models are typically parametric models -- so not as flexible

- Exercise: Consider what else should be in our model (not what data we have, what data would be ideal). Assume error is normally distributed and try and simulate a more sophisticated dataset for the case then what we did previously.
...
- Frequentist uncertainty is based on imaginary/theoretical resampling of data (sampling distributions)
- So only measurements -- estimators can have probability distributions (sampling)
- Bayesian uncertatinytreats randomness as a property of information, not the world
- Use randomness to describe our uncertainty in the face of incomplete knowledge
