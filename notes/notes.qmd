...
- Confusion and Penalized Regression (Jul 15)

### Hyperparameters

- Two definitions of hyperparameters: Prior predictive checks and tuning model parameters for prediction
...
- Include new overall model fit measures for discrete outcomes, likelihood ratio tests? ROC/AUC?
...
- PML p. 119 with the MLE + $\lambda C(\theta)$ where $\lambda$ is the regularization hyperparameter and $C(\theta)$ is the complexity penalty; for the Bayesian equivalent, $\lambda = 1$
...
- Another exercise where they use AI to help them evaluate what they're learning?
