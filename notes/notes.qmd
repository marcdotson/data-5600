...
- Finish simulating data for the case study
- Present as one big lecture, split into two days

## Assumptions

- Start with model assumptions in mind as we explore the data -- everything from plan and build -- use conditional analogy: Data | Model -> Model | Data?
- What diagnostics and transformations fit under each of these assumptions?
...
- EDA (Broken down by continuous and discrete variables?)
- Reference grammar of graphics
- Use data explorer in Positron (Assistant? Databot?)
- Reference earlier slides from Marketing Analytics, PyData blog post
...
Does Bambi have an easy way to do prior predictive checks?
...
- Activities, exercise to code with the dataset (explore and reconcile)
